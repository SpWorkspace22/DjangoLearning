...
from .models import Question, QuizProfile
...
@login_required
def play(request):
    quiz_profile, created = QuizProfile.objects.get_or_create(user=request.user.id)
...
def submission_result(request, attempted_question_pk):
    ...
    context = { 'attempted_question':attempted_question}
